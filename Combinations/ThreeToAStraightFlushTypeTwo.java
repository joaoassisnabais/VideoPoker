...
/**
 * Checks if there are two cards missing in the hand to get a Straight Flush (Type 2)
 * <p>Adds the cards that the player should keep to a list
 */
public class ThreeToAStraightFlushTypeTwo extends checkCombinations{

    /**
     * Constructor of the hand
     * @param hand hand of cards in analysis
     */
    public ThreeToAStraightFlushTypeTwo(Hand hand) {
        super(hand, 20);
    }

    /**
     * Checks if there here are two cards missing in the hand to get a Straight Flush ith one gap, or with two gaps and one
     ...
     * @return True if there here are two cards missing in the hand to get a Straight Flush ith one gap, or with two gaps and one
     * high card, or any ace-low, or 234 suited
     */
    @Override
    boolean isCombination() {
        ...
        for (int j=0; true; j++){
            ...
            if (k<=2 && ((highCardCount==1) || cardsExpected.get(0)==4)) { //a straight flush of type 2 was found
                break;
