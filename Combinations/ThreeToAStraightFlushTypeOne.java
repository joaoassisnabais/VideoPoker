...
/**
 * Checks if there are two cards missing in the hand to get a Straight Flush (Type 1)
 * <p>Adds the cards that the player should keep to a list
 */
public class ThreeToAStraightFlushTypeOne extends checkCombinations{

    /**
     * Constructor of the hand
     * @param hand hand of cards in analysis
     */
    public ThreeToAStraightFlushTypeOne(Hand hand) {
        super(hand, 14);
    }

    /**
     * Checks if there here are two cards missing in the hand to get a Straight Flush with a missing inside card
     * <p>Adds the three cards that will possibly make the Straight Flush to the list of cards that the player should keep
     * @return True if there are two inside cards missing in the hand to get a Straight Flush
     */
    @Override
    boolean isCombination() {
        ...
        for (int j=0; true; j++){
            ...
            if (k<=2 && highCardCount>=2) { //a straight flush of type 1 was found
                break;
